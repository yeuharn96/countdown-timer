...
class CountdownTimer:
    def __init__(self):
        ...
        # wrap setting values with tk.StringVar
        def wrap_stringvar(setting):
            for key in setting:
                if key == 'profile': continue # skip
                elif isinstance(setting[key], dict): wrap_stringvar(setting[key])
                else:
                    setting[key] = tk.StringVar(value=setting[key])
                    setting[key].trace('w', lambda *args: self.output.apply_output_config())
        profile.set_all(AppSetting.load(lambda: self.output.apply_output_config()))
        self.setting = profile.get_current()
        wrap_stringvar(self.setting)
        profile.get_current()
        # for key in self.setting:
        #     if isinstance(self.setting[key], tk.StringVar):
        #         self.setting[key].trace_id = self.setting[key].trace('w', lambda *args: self.output.apply_output_config())
                # self.setting[key].trace_vdelete('w', self.setting[key].trace_id)
                # self.setting[key].trace_remove('w', self.setting[key].trace_id)
    ...
    def init_root(self):
        def onExit():
            AppSetting.save(profile.get_all())
            self.root.destroy()
            exit()
    ...
    def init_widgets(self):
        ...
        display_frame.grid(row=1, column=0, sticky='w', padx=10, pady=5)
        ...
        setting_frame.grid(row=2, column=0, sticky='n')
        # set time frame
        time_frame = tk.Frame(setting_frame)
        time_frame.grid(row=0, column=0, sticky='n')
        def set_time(frame, var, btn_text, command):
            hour_options = [f'{i:02}' for i in range(0,24)]
            min_options = [f'{i:02}' for i in range(0,5)] + [f'{i:02}' for i in range(5,60,5)]
            frame = tk.Frame(frame, highlightbackground='black', highlightthickness=1)
            Dropdown(frame, title='hour', textvariable=var['hh'], width=5, options=hour_options).grid(row=0, column=0, padx=(10,5), pady=(0,10))
            Dropdown(frame, title='minute', textvariable=var['mm'], width=5, options=min_options).grid(row=0, column=1, padx=5, pady=(0,10))
            tk.Button(frame, text=btn_text, command=command).grid(row=0, column=2, padx=(5,10))
            return frame
        set_time(time_frame, timer.target['time'], btn_text='Set Time', command=lambda: timer.set_time()).pack(side='top', fill='x', pady=5, padx=10)
        set_time(time_frame, timer.target['duration'], btn_text='Set Duration', command=lambda: timer.set_duration()).pack(side='bottom', fill='x', pady=5, padx=10)

        # misc setting
        misc_frame = tk.Frame(setting_frame)
        misc_frame.grid(row=0, column=1, sticky='n', pady=(5,0))
        ...
        # title, font etc
        title_frame = tk.Frame(setting_frame)
        title_frame.grid(row=1, column=0, columnspan=2, sticky='wen', padx=10)
